...
func RefreshToken(ctx context.Context, grantType string, refreshToken string, scope string, clientId string, clientSecret string, host string) (interface{}, error) {
	...
	newAccessToken, newRefreshToken, tokenName, err := generateJwtToken(application, user, "", scope, host, oldToken.Sid)
	if err != nil {
		return &TokenError{
			Error:            EndpointError,
...
// GetTokenByUser
// Implicit flow
func GetTokenByUser(application *Application, user *User, scope string, host string, sid string) (*Token, error) {
	err := ExtendUserWithRolesAndPermissions(user)
	if err != nil {
		return nil, err
	}
